package kata1v2;

import java.util.Calendar;
import java.util.GregorianCalendar;

public class Person {
    private final String name;
    private final Calendar birhtday;
    private static final long MILLISECONDS_PER_YEAR =(long)(1000*60*60*24*365.25);

    public Person(String name, Calendar birhtday) {
        this.name = name;
        this.birhtday = birhtday;
    }

    public String getName() {
        return name;
    }

    public Calendar getBirhtday() {
        return birhtday;
    }

    public static long getMILLISECONDS_PER_YEAR() {
        return MILLISECONDS_PER_YEAR;
    }
    
    public int getAge(){
        Calendar today;
        today = GregorianCalendar.getInstance();
        return (int)(milliSecondsToYear(today.getTimeInMillis() - birhtday.getTimeInMillis()));
    }
    
    private long milliSecondsToYear(long milles){
        return milles/MILLISECONDS_PER_YEAR;
    }
}
